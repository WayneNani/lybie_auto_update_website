...
try:
    ...
    GIT_AUTHOR_EMAIL = CREDENTIALS.get("GIT_EMAIL")
    HUGO_BASE = CREDENTIALS.get("HUGO_BASE")
    ...
    db_connect_string = f'{CREDENTIALS.get("ORACLE_DB_USER")}/{CREDENTIALS.get("ORACLE_DB_PASSWORD")}' \
                        f'@{CREDENTIALS.get("ORACLE_DB_IP")}:1539/xepdb1'

    with oracle_connector.connect(db_connect_string) as con:
        last_access = oracle_connector.get_static_data(con, 'LAST_ACCESS')["date_value"]

        # Generate and write config.toml to disk
        config_content = file_builder.get_config_file_content(
            sidebar_text_de=oracle_connector.get_static_data(con, 'VORSTELLUNGSTEXT_DE')["varchar_value"],
            sidebar_text_en=oracle_connector.get_static_data(con, 'VORSTELLUNGSTEXT_EN')["varchar_value"],
            template=oracle_connector.get_template(con, 'CONFIG')[0])
        ...
        location = file_builder.get_sidebar_file_content(
            location=oracle_connector.get_static_data(con, 'AUFENTHALTSORT')["varchar_value"],
            travel_date=oracle_connector.get_static_data(con, 'REISEDATUM')["varchar_value"],
            template=oracle_connector.get_template(con, 'SIDEBAR')[0])
        ...
        for place in oracle_connector.get_place_data(con, last_access):
            images_file_names = []
            place_template = oracle_connector.get_template(con, 'PLACE')[0]

            # Save Thumbnail
            if place["id_thumbnail"]:
                thumbnail = oracle_connector.get_image(con, place["id_thumbnail"])
                try:
            ...
            try:
                images_file_names = util.save_files_to_disk(oracle_connector.get_images_for_place(con, place["id"]),
                                                            HUGO_BASE,
                                                            con)
            except Exception as e:
            ...
            place_content_de = file_builder.get_place_file_content(text=place["text_de"],
                                                                   location=place["location_de"],
                                                                   creation_date=place["creation_date"].strftime(
                                                                       "%Y-%m-%d"),
                                                                   thumbnail=thumbnail_name,
                                                                   template=place_template,
                                                                   images=images_file_names)

            place_content_en = file_builder.get_place_file_content(text=place["text_en"],
            ...
            try:
                file_builder.write_file(path=os.path.join(HUGO_BASE, 'content',
                                                          'portfolio', place["place_name"] + '.de.md'),
                                        mode='w', content=place_content_de)

                file_builder.write_file(path=os.path.join(HUGO_BASE, 'content',
                                                          'portfolio', place["place_name"] + '.md'),
    ...
    if repo.is_dirty() and not errors:
        ...
        if many_images:
            ...
            util.write_files_to_sftp(CREDENTIALS.get('SFTP_SERVER'),
                                     CREDENTIALS.get('SFTP_USER'),
                                     CREDENTIALS.get('SFTP_PASSWORD'))
    ...
    util.print_error('"credentials.json" konnte nicht gefunden werden...')
